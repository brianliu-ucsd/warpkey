...
| 19 | Leader key is editable ("Key model") | In the popup, click the leader-key `kbd` in the header (shows `` ` `` by default), then press a different key, e.g. `j`. Open the fixture page's devtools console and check for a `Warpkey: leader key set to ...` line (logged as soon as the change propagates, no reload needed). Try `` ` `` then an existing binding's key, then try `j` then that same key - as two separate presses, not simultaneous. | The console log confirms the new key arrived. `` ` `` no longer arms the chord; `j` does (both need a second keypress for the bound key - the leader alone only opens the which-key overlay). Reopening the popup shows `j` as the current leader key. |
| 20 | Popup and overlay labels reflect live state ("Display labels") | Record a binding on the **TestID-tier button** (as in #1), fire it a couple more times (count should be at 2+), then reopen the popup and separately press the leader key alone to check the which-key overlay. | Both the popup's binding row and the overlay's list show the button's actual current count (e.g. "clicks: 2"), not the frozen "clicks: 0" captured at record time. |
