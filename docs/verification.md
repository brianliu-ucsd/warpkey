...
| 19 | Leader key is editable ("Key model") | In the popup, click the leader-key `kbd` in the header (shows `` ` `` by default), then press a different key, e.g. `j`. Reload the fixture page. Try `` ` `` then an existing binding's key, then try `j` then that same key. | `` ` `` no longer arms the chord; `j` does. Reopening the popup shows `j` as the current leader key. |
| 20 | Popup label doesn't misrepresent live state ("Storage") | Record a binding on the **TestID-tier button** (as in #1), fire it a couple more times, then reopen the popup. | The binding's label shows a normalized placeholder (e.g. "clicks: #") rather than a frozen "clicks: 0" that stops matching reality after the first fire. |
