...
| 16 | **TODO - not currently testable.** Multi-device sync | Sign into the same Chrome profile on a second machine (or a second Chrome profile), record a binding on one, check the popup on the other. | Binding appears after `chrome.storage.sync` propagates (may take a few seconds). Not automatable - needs two real profiles/machines, unavailable in the current test setup. |
